...
- **オクターブ拘束**: GUI は検出を期待 BPM ±35%（`tempo_lock_range_pct`）に拘束し、2倍/半分の誤検出を排除（In Bloom で 156→78 に是正）。CLI は `--expected-bpm`/`--seed` 指定時に自動有効、`--lock-range` で調整可。
- **ズレログ**: 検出が期待 BPM から既定 8%（`deviation_log_pct`）超ズレると `logs/deviation_*.csv` に記録（曲調チェンジでの減速などを後で分析。オクターブ拘束のおかげで本物の帯域内ズレだけが残る）。ズレ中は画面に⚠表示。閾値が敏感すぎる場合は `deviation_log_pct` を上げる。
